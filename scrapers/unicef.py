...
def scrape_unicef():
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"UNICEF: Status {res.status_code}")
            return opportunities

        soup = BeautifulSoup(res.text, "lxml")

        items = soup.select("div.procurement, article, .notice, .tender")
        ...
        if not items:
            ...
            for link in links:
                ...
                if href and not href.startswith("http"):
                    href = "https://www.unicef.org" + href
        ...
        print(f"UNICEF scraping error: {e}")
...
if __name__ == "__main__":
    data = scrape_unicef()
    print(f"Found {len(data)} opportunities from UNICEF")
