...
URL = "https://en.unesco.org/career"
...
def scrape_unesco():
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"UNESCO: Status {res.status_code}")
            return opportunities

        soup = BeautifulSoup(res.text, "lxml")

        items = soup.select("div.vacancy, article, .job, .opportunity")
        ...
        print(f"UNESCO scraping error: {e}")
