...
def scrape_tendersinfo():
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"TendersInfo: Status {res.status_code}")
            return opportunities

        soup = BeautifulSoup(res.text, "lxml")

        items = soup.select(
            "tr.data-row, div.tender, article, div.tender-card, .tenders-list-item"
        )
        ...
        if not items:
            ...
            for link in links:
                ...
                if href and not href.startswith("http"):
                    href = "https://www.tendersinfo.com" + href
            ...
            for item in items[:30]:
                ...
                country_elem = item.select_one(".country, td:nth-child(2)")
        ...
        print(f"TendersInfo scraping error: {e}")

    return opportunities


if __name__ == "__main__":
    data = scrape_tendersinfo()
    print(f"Found {len(data)} opportunities from TendersInfo")
