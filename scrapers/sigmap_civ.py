...
def scrape_sigmap_civ():
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"SIGMAP: Status {res.status_code}")
            return opportunities

        soup = BeautifulSoup(res.text, "lxml")

        items = soup.select("div.marche, article, .appel-offre, .tender, .marche-item")
                ...
                opportunities.append(
                    {
                        "title": title,
                        "description": title,
                        "organization": "SIGMAP Côte d'Ivoire",
        ...
        print(f"SIGMAP scraping error: {e}")

    return opportunities


if __name__ == "__main__":
    data = scrape_sigmap_civ()
    print(f"Found {len(data)} opportunities from SIGMAP")
